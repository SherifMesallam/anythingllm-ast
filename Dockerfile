...
# MODIFIED: Use $ARG_UID:$ARG_GID in chown
RUN (groupadd -g "$ARG_GID" anythingllm || true) && \
    useradd -l -u "$ARG_UID" -m -d /app -s /bin/bash -g "$ARG_GID" anythingllm && \
    mkdir -p /app/frontend/ /app/server/ /app/collector/ && chown -R "$ARG_UID":"$ARG_GID" /app
...
COPY --chown="$ARG_UID":"$ARG_GID" ./frontend /app/frontend/
...
COPY --chown="$ARG_UID":"$ARG_GID" ./server /app/server/
WORKDIR /app/server
RUN yarn install --production --network-timeout 100000 && yarn cache clean
WORKDIR /app

# Install collector dependencies
COPY --chown="$ARG_UID":"$ARG_GID" ./collector/ ./collector/
...
WORKDIR /app
COPY --chown="$ARG_UID":"$ARG_GID" --from=frontend-build /app/frontend/dist /app/server/public
USER root
RUN chown -R "$ARG_UID":"$ARG_GID" /app/server && \
    chown -R "$ARG_UID":"$ARG_GID" /app/collector
USER anythingllm
